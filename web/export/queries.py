"""Couche SQL de l'export web — les agrégations sont faites par DuckDB, pas en pandas.

L'entrepôt `housing_data/` expose déjà les datasets en Parquet typé avec une vue SQL
DuckDB par dataset (zéro serveur). Ce module est la porte d'entrée unique de
`web_export.py` : il ouvre cette connexion et fait faire au moteur ce qui était
auparavant refait à la main côté Python — GROUP BY, cumuls glissants (fonctions de
fenêtre), YoY par LAG, z-score, jointures, filtrage des NULL. Les builders d'onglets
ne reçoivent plus que des lignes déjà agrégées, prêtes à sérialiser.
...
Deux formes de sortie :
  ...
  * `frame(...)` -> DataFrame, pour les quelques helpers partagés avec `app.py`
    (`analysis.calculate_kpis` / `momentum_metrics`) qu'on réutilise tels quels — c'est
    le contrat de la migration : on ne duplique pas la logique métier de l'app.
...
def open_warehouse(refresh: bool = True):
    ...
    où pandas voit les données brutes. Tout le reste de l'export passe par SQL.
    """
    if refresh:
